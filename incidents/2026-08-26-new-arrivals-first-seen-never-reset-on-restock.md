# New Arrivals / Sold Out Silently Broke After Long Stock Gaps

**Date:** 2026-08-26
**Repos:** `mnlegitdev/legit-cannabis-south-metro-menu`, `mnlegitdev/dinky-dope-menu`, `capitanminovel/legit-buddy-api`, `capitanminovel/dinky-buddy-api`
...
- **Also ported to the stale `capitanminovel` pre-handoff copies** (2026-08-26, same day, on request): per `[[mnlegitdev-repo-handoff]]`, `capitanminovel/legit-buddy-api` and `capitanminovel/dinky-buddy-api` are *not* forks of the mnlegitdev repos but independent copies that still run their own `daily-scrape.yml` against the same two physical stores (south-metro and Dinkytown respectively) — confirmed both had scraped as recently as the same day. Their `scraper.py` had diverged further than dinky-dope-menu's (no id/legacy-key split at all — `legit-buddy-api` reverted its stable-id key scheme entirely per an earlier commit, "no stable POS product ID available"; both use a single text-hash `product_key()`), so the fix was adapted to their single-branch `merge()` rather than copy-pasted, but the same `RESTOCK_GAP_DAYS`/`_is_restock` logic applies unchanged. Commits: `297b322` (legit-buddy-api), `5d602fd` (dinky-buddy-api). Full-history audits found the same real-world restock events as their mnlegitdev counterparts (same physical stores → same data): legit-buddy-api backfilled 10 products (6 in today's batch: Fight Club, LIT OG, MAC Stomper, Runtz, Permanent Marker, PR Lit OG; 4 backdated: Cap Junky, Soap, PR Medusa 2pk (0.5 G ea), Illemonati); dinky-buddy-api backfilled 4 (2 today: Runtz, Fight Club; 2 backdated: Purple Octane, Blueberry Muffin 5ct).
- Same standing note as the prior incident: independently-maintained near-identical/near-duplicate repos means every `scraper.py` bug gets found and fixed up to four times now (two mnlegitdev + two capitanminovel copies, all scraping the same two physical stores). Still not acted on — still worth a conversation with the owner about consolidating, and about whether the stale `capitanminovel` copies should finally be retired now that both are confirmed still actively scraping and serving their own live Pages sites.
